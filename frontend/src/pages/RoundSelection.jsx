...
export default function RoundSelection() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:5000/api/candidate/${candidateId}/rounds-completed`)
      .then((res) => setCompletedData(res.data.rounds_completed))
      .catch(() => setCompletedData([]));
  }, [candidateId]);

  const getSession = (key) => completedData.find((r) => r.round_type === key);
  const canViewCombined = completedData.length >= 2;
    ...
    <div className="min-h-screen bg-[var(--color-bg)] px-6 py-16">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-10">
          {ROUNDS.map((r) => {
            const session = getSession(r.key);
            ...
            return (
              <div key={r.key} className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-6 flex flex-col">
                ...
                <button
                  onClick={() => {
                    if (isDone) {
                      navigate(`/report/round/${session.session_id}`);
                    } else {
                      navigate(r.path);
                    }
                  }}
                ...
                >
                  {isDone ? 'View Report' : 'Start Round'}
                </button>
              </div>
            );
          })}
